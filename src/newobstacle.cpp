...
public:
    ...
    bool init(int send_or_not)
    {
        ...
        if (send_or_not == 1) {
            ...
            std::cout << "Created data writer .... " << std::endl;
        }
        else {
    ...
    bool publish(newBlackboard *bb, sem_t *sem, int obs_x[100], int obs_y[100])
    {
        ...
        if (publistener_.matched_ > 0)
        {
            ...
            for (int i=0; i<bb->n_obstacles; i++){
                std::cout << "Obstacle " << i << " x: " << obs_x[i] << " y: " << obs_y[i] << std::endl;
            }
            // sem_post(sem);
            ...
            std::vector<int32_t> obstacles_x(obs_x, obs_x + 5);
            std::vector<int32_t> obstacles_y(obs_y, obs_y + 5);
